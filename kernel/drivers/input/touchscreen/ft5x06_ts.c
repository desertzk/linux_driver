 ...
 *
 * Copyright (c) 2012 GEC (www.gec-lab.org)
 *
...
#ifdef CONFIG_TOUCHSCREEN_FT5X0X_SINGLE	//add
...
static int ft5x0x_write_reg(u8 addr, u8 val) {
	...
	if (ret < 0) {
		printk("write 0x%02x to reg (0x%02x) failed, %d", addr, val, ret);
		return -1;
	}
...
static void ft5x0x_ts_report(struct ft5x0x_ts_data *ts) {
	...
	if (event->touch_point == 1) {
		...
		//printk("max->x = %d,TOUCH_MAX_X=%d,x=%d\n",ts->screen_max_x,TOUCH_MAX_X,x);
		//printk("max->y = %d,TOUCH_MAX_y=%d,y=%d\n",ts->screen_max_y,TOUCH_MAX_Y,y);
...
static int ft5x0x_read_data(struct ft5x0x_ts_data *ts) {
	...
	switch (event->touch_point) {
		case 5:
			...
			event->y[4] = 480 - ((s16)(buf[0x1d] & 0x0F)<<8 | (s16)buf[0x1e]);
		case 4:
			event->x[3] = (s16)(buf[0x15] & 0x0F)<<8 | (s16)buf[0x16];
			event->y[3] = 480 - ((s16)(buf[0x17] & 0x0F)<<8 | (s16)buf[0x18]);
		case 3:
			event->x[2] = (s16)(buf[0x0f] & 0x0F)<<8 | (s16)buf[0x10];
			event->y[2] = 480 - ((s16)(buf[0x11] & 0x0F)<<8 | (s16)buf[0x12]);
		case 2:
			event->x[1] = (s16)(buf[0x09] & 0x0F)<<8 | (s16)buf[0x0a];
			event->y[1] = 480 - ((s16)(buf[0x0b] & 0x0F)<<8 | (s16)buf[0x0c]);
		case 1:
			event->x[0] = (s16)(buf[0x03] & 0x0F)<<8 | (s16)buf[0x04];
			event->y[0] = 480 - ((s16)(buf[0x05] & 0x0F)<<8 | (s16)buf[0x06]);
			break;
		default:
...
static int ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (pdata->gpio_cfg) {
		//gpio_direction_output(ts->gpio_irq,0);
		//gpio_set_value(ts->gpio_irq, 0x00);
	}
	...
	err = request_irq(client->irq, ft5x0x_ts_interrupt,
			  /*IRQF_TRIGGER_RISING*/IRQ_TYPE_EDGE_FALLING/*IRQF_TRIGGER_FALLING*/,
			  "ft5x0x_ts", ts);
	if (err < 0) {
...
MODULE_DEVICE_TABLE(i2c, ft5x0x_ts_id); //i2c设备列表

static struct i2c_driver ft5x0x_ts_driver = {
	.probe		= ft5x0x_ts_probe, //文件操作函数
	.remove		= __devexit_p(ft5x0x_ts_remove), //remove函数
	.id_table	= ft5x0x_ts_id,                  //id号
	.driver	= {
		.name	= FT5X0X_NAME,
		.owner	= THIS_MODULE,
	},
...
{
	return i2c_add_driver(&ft5x0x_ts_driver); //添加一个设备驱动文件
}

static void __exit ft5x0x_ts_exit(void)
{
	i2c_del_driver(&ft5x0x_ts_driver);//删除设备文件
}
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:ft5x0x_ts_data");
