	...
	case STATE_WRITE:
		...
		if (!(i2c->msg->flags & I2C_M_IGNORE_NAK)) {
			if (iicstat & S3C2410_IICSTAT_LASTBIT) {
				...
				if(i2c->pdata->bus_num == 1){
					gpio_free(zr_rest);
					gpio_request(zr_rest, "RELAY");
					gpio_direction_output(zr_rest, 0);
					msleep(10);
					gpio_direction_input(zr_rest);
				}
...
static int s3c24xx_i2c_probe(struct platform_device *pdev)
{
	...
	//i2c->clk = clk_get(&pdev->dev, "nxp-i2c");
